...
function html$1(strings, ...values) {
  ...
  const isDynamicProp = p =>
    p && p.d && typeof p.d === "string" && Object.keys(p).length === 1
...
function isValidRenderArg(x) {
  return isIterable$1(x)
    ? [...x].every(isValidNonIterableRenderArg)
    : isValidNonIterableRenderArg(x)
}
 ...
 * @returns {T}
 */
function litFixtureSync(template) {
 ...
 * @returns {Promise<T>}
 */
async function litFixture(template) {
 ...
 * @returns {T}
 */
function stringFixtureSync(template) {
  const wrapper = fixtureWrapper()
